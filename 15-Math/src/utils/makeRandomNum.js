...
export const makeRandomNum = () => Math.floor(Math.random() * (MAX_NUMBER - MIN_NUMBER) + MIN_NUMBER)

export const derivingCorrectAnswer = (operator) => {
    const firstNum = makeRandomNum()
    const secondNum = makeRandomNum()

    const correctAnswer =  parseFloat(eval(`${firstNum} ${operator} ${secondNum}`).toFixed(1))

    return correctAnswer < 0 ? derivingCorrectAnswer(operator) : [ firstNum, secondNum, correctAnswer ]
}

export const randomAnswerArray = (correctAnswer, operator) => {
    const answerList = [ correctAnswer, derivingCorrectAnswer(operator)[2], derivingCorrectAnswer(operator)[2] ]
